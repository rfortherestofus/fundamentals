...
## Make your variables easy to work with

```{r eval = T, echo = F}
bad_names <- read_csv(here("data/badnames.csv"))
```
...
```{r eval = T, echo = F}
good_names <- read_csv(here("data/badnames.csv")) %>% 
  clean_names()
```

```{r}
good_names <- read_csv("data/badnames.csv") %>% 
  clean_names() #<<
```
