...
Do nearly the same thing to change the color of the last bar chart you made (the one about sleep and gender). Note that NA values will not show up so use the `na.value` argument to set the NAs to be blue (you can use the text "blue" to do this)
...
1. Use the `round` argument to just show one decimal place in each label.
1. Use the `vjust` argument to have them show up at the inner edge of the bars.
1. Make the labels all white.
...
1. Don't show the legend (hint: you'll have to add code in two places to make this happen).
...
1. Add a title
1. Add a better y axis label
1. Remove the x axis label
1. Remove the legend
